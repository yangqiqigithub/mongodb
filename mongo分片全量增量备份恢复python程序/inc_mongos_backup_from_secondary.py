    ...
    def recoverMongosAfterBackup(self):
        ...
        for second in self.second_node_used.itervalues():
            ...
            cmd_permit_write = self.config.mongo_bin_dir + "/mongo "+  \
                               str(second) + "/admin "   \
                               " --quiet " +                \
                               " --eval 'rs.slaveOk();db.fsyncUnlock();'"+\
	    ...
	    #print("cmd_permit_write:",cmd_permit_write)
            ...
            #print("commands.getstatusoutput(cmd_permit_write):",commands.getstatusoutput(cmd_permit_write))
            #print("status:",status)
            #print("unLocked",unLocked)
            if status != 0 or not unLocked:
                print ('fsncUnlock '+ second + ' fail!')
            else:
                print('fsncUnlock '+ second + ' successful!')
    ...
    def backupConfig(self):
        if check_dir(self.config.full_backup_dir):
            ...
            cmd_line = self.config.mongo_bin_dir + "/mongodump"    \
                      ...
                      " --out "  + backup_dir
           # print "configbackupcmd",cmd_line
            status, output = commands.getstatusoutput(cmd_line)
            if status != 0 or not output:
                print ("backup config " + str(self.config.config_ip) + ':' + str(self.config.config_port) + " failed!")
                return False
            print ('finished backup config ' + str(self.config.config_ip) + ':' + str(self.config.config_port) + '.')
            return True
    ...
    def fullBackupShard(self):
         ...
         if check_dir(dir):
             for shard in self.shard_info:
                ...
                #print "lalala",cmd_last_oplog_record
                # #
...
def Launcher(path, backType):
    ...
    if back.getShardInfo():
        ...
        if back.prepareForBackup():
            ...
            if backType == 'full':
                if back.backupConfig() and back.fullBackupShard() and back.incBackupShard():
